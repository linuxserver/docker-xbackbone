pipeline {
  ...
  environment {
    ...
    EXT_USER = 'SergiX44'
    EXT_REPO = 'XBackBone'
    BUILD_VERSION_ARG = 'XBACKBONE_RELEASE'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-xbackbone'
    CONTAINER_NAME = 'xbackbone'
    DOCKERHUB_IMAGE = 'linuxserver/xbackbone'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/xbackbone'
    PR_DOCKERHUB_IMAGE = 'lspipepr/xbackbone'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='true'
    CI_WEB='true'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/xbackbone") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/xbackbone",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-xbackbone",\
                       "backendUrl":"https://ghcr.io/linuxserver/xbackbone",\
                       "publicUrl":"https://lscr.io/linuxserver/xbackbone"}' || :
              else
